...
export function getRelatedTools(currentTool: string) {
  const tool = toolRelationships[currentTool as keyof typeof toolRelationships];
  if (!tool) return [];
  
  return tool.related.map(relatedKey => toolRelationships[relatedKey as keyof typeof toolRelationships]).filter(Boolean);
}
